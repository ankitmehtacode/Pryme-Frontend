...
import { Shield, Lock, Trash2, EyeOff, Info } from "lucide-react";
...
// 🧠 The old Radix Tooltip only opened on hover, which doesn't exist on
// touch devices -- there was no way to see the explanation on mobile at
// all. Click-to-expand works identically on both desktop and mobile (a
// tap IS a click), so it replaces hover entirely instead of trying to
// bolt a second touch-specific mechanism onto Radix's hover-only Tooltip.
const TiltCard = ({ feature, index }: { feature: any; index: number }) => {
  ...
  return (
    <motion.div
      ...
      className="relative group bg-white/40 dark:bg-slate-900/40 backdrop-blur-md rounded-2xl border border-slate-200/40 dark:border-white/5 p-5 transition-all duration-300 cursor-pointer text-left z-10
    ...
    >
      ...
      <div className="flex items-start justify-between">
        ...
        <Info className={`w-4 h-4 mt-1 transition-colors duration-300 ${expanded ? "text-[#103783] dark:text-blue-400" : "text-slate-300 dark:text-slate-600"}`} />
      </div>
      ...
      <AnimatePresence initial={false}>
        {expanded && (
          ...
          >
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed pt-3 border-t border-slate-200/60 dark:border-white/10">
              {feature.tooltip}
            </p>
          </motion.div>
        )}
