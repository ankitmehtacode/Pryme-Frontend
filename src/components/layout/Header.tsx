          ...
          <div>
            ...
            <div className="space-y-1">
              {productLinks.map((item) => (
                <Link key={item.href} to={item.href} onClick={onClose} className="flex items-center gap-3 p-3 rounded-lg hover:bg-emerald-50 transition-colors">
                  <div className="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center"><item.icon className="w-4 h-4 text-[#2aac64]" /></div>
      ...
      <header ref={headerRef} className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-0 transition-all duration-300 pointer-events-none">
        <div ref={navContainerRef} className="w-full h-20 px-6 flex items-center justify-between transition-all duration-300 pointer-events-auto bg-transparent">
          ...
          <nav className="hidden lg:flex items-center gap-1 bg-white/50 rounded-full px-2 py-1 border border-white/20 backdrop-blur-sm shadow-sm">
                  ...
                  <NavigationMenuContent>
                    <div className="w-[500px] p-4 bg-white rounded-2xl shadow-xl border border-slate-100">
                      <div className="grid grid-cols-2 gap-2">
                        {productLinks.map((item) => (
                          <Link key={item.href} to={item.href} className="flex items-start gap-3 p-3 rounded-xl hover:bg-emerald-50 transition-colors group">
                            <div className="w-10 h-10 rounded-lg bg-emerald-50 flex items-center justify-center shrink-0"><item.icon className="w-5 h-5 text-[#2aac64]" /></div>
            ...
            {navLinks.map(link => (
              <Link key={link.href} to={link.href} className={cn("px-4 py-2 text-sm font-medium rounded-full transition-all hover:bg-black/5", location.pathname === link.href ? "text-[#2aac64]" : "text-slate-600")}>
                {link.label === "Home" ? (
          ...
          <div className="hidden lg:flex items-center gap-4">
            <a href={CONTACT_PHONE_LINK} className="flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-[#2aac64] transition-colors"><Phone className="w-4 h-4" /><span>{CONTACT_PHONE}</span></a>
            {user ? (
              <DropdownMenu>
