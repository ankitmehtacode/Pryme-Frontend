...
import { Phone, CheckCircle2, Loader2, RotateCw, ShieldAlert } from "lucide-react";
 ...
 *    that token is what submission is checked against.
...
export const MobileOtpVerifier: React.FC<Props> = ({
  ...
  // A changed number invalidates any session in flight -- the code was sent to
  // the old handset and must not appear to apply to the new one.
  useEffect(() => {
  ...
  const requestCode = useCallback(
    async (isResend: boolean) => {
      if (!isValidMobile(mobileNumber) || disabled) return;
      setPhase("sending");
      setError(null);
      try {
        const res = isResend
          ? await PrymeAPI.resendMobileOtp(mobileNumber)
          : await PrymeAPI.sendMobileOtp(mobileNumber);
        setSession(res);
  ...
  const setDigit = (index: number, value: string) => {
    ...
    // actually move a code across from their SMS app.
    if (clean.length > 1) {
  ...
  const helper = useMemo(() => {
    if (expired) return "This code has expired. Request a new one.";
    if (phase === "entry") return `Code sent to ${mobileNumber}. Valid for ${mmss(expiresIn)}.`;
    return null;
  }, [expired, phase, mobileNumber, expiresIn]);
  ...
  if (!isValidMobile(mobileNumber)) return null;
  ...
  return (
    <div className="animate-in fade-in slide-in-from-top-1 mt-1 space-y-2">
      {phase === "idle" || phase === "sending" ? (
        ...
        >
          {phase === "sending" ? (
            ...
            <><Phone className="w-3 h-3 mr-1.5" /> Verify</>
          )}
        </Button>
      ) : (
        <>
          <div className="flex items-center gap-2" role="group" aria-label="Enter the code sent by SMS">
      ...
      {helper && !error && (
        <p className="text-[10px] text-muted-foreground/60 ml-0.5 tabular-nums" aria-live="polite">
          {helper}
        </p>
      )}

      {error && (
        <p className="text-[11px] font-medium text-red-500 flex items-start gap-1 ml-0.5" aria-live="assertive">
          <ShieldAlert className="w-3.5 h-3.5 shrink-0 mt-px" />
          <span>
            {error}
            {attemptsLeft != null && attemptsLeft > 0 && phase === "entry" && !expired && (
              <span className="text-muted-foreground/70"> {attemptsLeft} attempt{attemptsLeft === 1 ? "" : "s"} left.</span>
            )}
          </span>
        </p>
      )}
