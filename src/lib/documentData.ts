...
export type ProductType = "Home Loan" | "LAP" | "Business Loan" | "Personal Loan" | "Auto Loan";
...
const buildHomeLoanDocs = (empType: EmploymentType): DocumentItem[] => {
  ...
  if (empType === "Salaried") {
    docs.push(
      ...
      { id: "hl_sal_itr", label: "ITR (if applicable)", category: "Income", optional: true }
    );
  } else {
    docs.push(
...
const buildLAPDocs = (empType: EmploymentType): DocumentItem[] => {
  ...
  // Income Documents
  docs.push(
    { id: "lap_itr", label: "ITR (last 2–3 years)", category: "Income" },
    { id: "lap_bspl", label: "Balance Sheet & P&L", category: "Income" },
    { id: "lap_bank", label: "Bank statements (6–12 months)", category: "Income" }
  );
  ...
  docs.push(
    ...
    { id: "lap_occ", label: "Occupancy certificate (if applicable)", category: "Property", optional: true }
  );

  // Business Proof (if applicable)
  if (empType !== "Salaried") {
    docs.push(
      { id: "lap_gst", label: "GST Certificate", category: "Business Proof", optional: true },
      { id: "lap_udyam", label: "Udyam Registration / Shop Act", category: "Business Proof", optional: true },
...
const buildBusinessLoanDocs = (): DocumentItem[] => {
  ...
  // KYC Documents
  docs.push(
    { id: "bl_pan", label: "PAN (Individual + Business)", category: "KYC" },
  ...
  docs.push(
    ...
    { id: "bl_gst_returns", label: "GST Returns (6–12 months)", category: "Financial" },
...
const buildPersonalLoanDocs = (empType: EmploymentType): DocumentItem[] => {
  ...
  if (empType === "Salaried") {
    docs.push(
      ...
      { id: "pl_form16", label: "Form 16 (if required)", category: "Income", optional: true }
    );
  } else {
    docs.push(
      { id: "pl_se_itr", label: "ITR (1–2 years)", category: "Income" },
      { id: "pl_se_bank", label: "Bank statements", category: "Income" },
      { id: "pl_se_gst", label: "GST (if applicable)", category: "Income", optional: true }
    );
  }
...
const buildAutoLoanDocs = (empType: EmploymentType): DocumentItem[] => {
  const docs: DocumentItem[] = [];

  // KYC Documents
  docs.push(
    { id: "al_pan", label: "PAN", category: "KYC" },
    { id: "al_aadhar", label: "Aadhaar Card / Passport / Voter ID / Driving License", category: "KYC" },
    { id: "al_photo", label: "Passport size photographs", category: "KYC" }
  );

  // Income Documents
  if (empType === "Salaried") {
    docs.push(
      { id: "al_sal_slips", label: "Last 3 months salary slips", category: "Income" },
      { id: "al_sal_bank", label: "Last 6 months bank statement (salary account)", category: "Income" },
      { id: "al_form16", label: "Form 16 (last 2 years)", category: "Income" },
      { id: "al_sal_itr", label: "ITR (if applicable)", category: "Income", optional: true }
    );
  } else {
    docs.push(
      { id: "al_se_itr", label: "ITR (last 2–3 years) with computation", category: "Income" },
      { id: "al_se_bspl", label: "Balance Sheet & P&L (CA certified)", category: "Income" },
      { id: "al_se_bizproof", label: "Business proof (GST / Shop Act / MSME Registration)", category: "Income" },
      { id: "al_se_bank", label: "Last 6–12 months bank statements (business + personal)", category: "Income" }
    );
  }

  // Additional
  docs.push(
    { id: "al_quotation", label: "Quotation of vehicle (IMP)", category: "Additional" },
    { id: "al_cc_stmt", label: "Credit card statements (if required)", category: "Additional", optional: true },
    { id: "al_existing_loan", label: "Existing loan statements (if any)", category: "Additional", optional: true }
  );
