        ...
        <div className="max-w-6xl mx-auto relative z-10 space-y-12">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-4">
            ...
            <p className="text-muted-foreground text-lg md:text-xl max-w-2xl mx-auto font-light">
              Based on your verified CIBIL of <strong className="text-foreground">{leadData.cibilScore}</strong>, we have locked in {dynamicOffers.length} institutional offers for ₹{leadData.loanAmount.toLocaleString('en-IN')}.
            </p>
          </motion.div>

          {/* THE HERO OFFER */}
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2, type: "spring" }} className="relative p-[1px] rounded-[2rem] bg-gradient-to-b from-primary/50 to-border/30 overflow-hidden group">
            <div className="absolute inset-0 bg-primary/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700 blur-2xl" />
            <div className="bg-card rounded-[2rem] p-6 md:p-10 relative h-full w-full flex flex-col md:flex-row gap-8 items-center justify-between">

              <div className="flex-1 space-y-6 w-full">
                <div className="flex items-center gap-4">
                  <div className={`w-14 h-14 rounded-2xl ${heroOffer.logoColor} flex items-center justify-center shadow-lg`}>
                    <Building2 className="w-7 h-7 text-white" />
                  </div>
                  <div>
                    <div className="flex items-center gap-3">
                      <h2 className="text-2xl font-semibold text-foreground">{heroOffer.bankName}</h2>
                    ...
                    <p className="text-emerald-500 text-sm font-medium flex items-center gap-1.5 mt-1">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-border">
                  <div>
                    <p className="text-muted-foreground text-xs font-medium uppercase tracking-widest mb-1">Interest Rate</p>
                    <p className="text-2xl font-semibold flex items-center gap-1 text-foreground">{heroOffer.interestRate}<Percent className="w-4 h-4 text-muted-foreground" /></p>
                  </div>
                  <div>
                    <p className="text-muted-foreground text-xs font-medium uppercase tracking-widest mb-1">Monthly EMI</p>
                    <p className="text-2xl font-semibold flex items-center gap-1 text-foreground"><IndianRupee className="w-4 h-4 text-muted-foreground" />{calculateEMI(leadData.loanAmount, heroOffer.interestRate, heroOffer.maxTenure).toLocaleString('en-IN')}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground text-xs font-medium uppercase tracking-widest mb-1">Max Tenure</p>
                    <p className="text-2xl font-semibold flex items-center gap-1 text-foreground">{heroOffer.maxTenure} <span className="text-sm text-muted-foreground font-normal">Yrs</span></p>
                  </div>
                  <div>
                    <p className="text-muted-foreground text-xs font-medium uppercase tracking-widest mb-1">Processing</p>
                    <p className="text-2xl font-semibold flex items-center gap-1 text-foreground">{heroOffer.processingFee}<Percent className="w-4 h-4 text-muted-foreground" /></p>
                  </div>
                </div>
              </div>

              <div className="w-full md:w-auto md:min-w-[240px] flex flex-col items-center justify-center p-6 rounded-3xl bg-secondary/50 dark:bg-white/5 border border-border shrink-0">
                ...
                <p className="text-[10px] text-muted-foreground uppercase tracking-widest mt-4 flex items-center gap-1"><Clock className="w-3 h-3" /> Expires in 24h</p>
              </div>
          ...
          <div className="space-y-6 pt-8">
            <h3 className="text-lg font-medium text-muted-foreground flex items-center gap-2">
              <span className="w-8 h-[1px] bg-border" /> Alternative Matrix Options
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {alternativeOffers.map((offer, index) => (
                <motion.div key={offer.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + (index * 0.1) }} className="bg-card border border-border hover:border-primary/20 rounded-2xl p-6 transition-colors group flex flex-col">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-semibold text-foreground">{offer.bankName}</h4>
                        <p className="text-xs text-muted-foreground">{offer.approvalOdds}% Approval Odds</p>
                      </div>
                    </div>
                    {offer.tag && <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground border border-border px-2 py-0.5 rounded">{offer.tag}</span>}
                  </div>
                  <div className="space-y-4 mb-6 flex-1">
                    <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Interest Rate</span><span className="font-semibold text-foreground">{offer.interestRate}%</span></div>
                    <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Processing Fee</span><span className="font-semibold text-foreground">{offer.processingFee}%</span></div>
                    <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">EMI ({offer.maxTenure} yrs)</span><span className="font-semibold text-foreground">₹{calculateEMI(leadData.loanAmount, offer.interestRate, offer.maxTenure).toLocaleString('en-IN')}</span></div>
                  </div>
                  <Button onClick={() => handleSelectOffer(offer)} disabled={isLocking !== null} variant="outline" className="w-full bg-transparent border-border text-muted-foreground hover:text-foreground hover:bg-secondary rounded-xl transition-all">
